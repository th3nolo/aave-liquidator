...
function parseUsers(payload) {
  ...
  loans = loans.filter(loan => {
    const token = TOKEN_LIST[loan.max_borrowedSymbol];
    if (!token) {
      console.error(`Token not found: ${loan.max_borrowedSymbol}`);
      return false;
    }
    return loan.max_borrowedPrincipal * allowedLiquidation * (loan.max_collateralBonus - 1) * loan.max_borrowedPriceInEth / 10 ** token.decimals >= profit_threshold;
  });
  return loans;
}
